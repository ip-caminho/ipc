...
function Conteudo({ acampamentoId }: { acampamentoId: Id<"acampamentos"> }) {
  ...
  const inscricoes = useQuery(api.acampamento.queries.listarInscricoes, { acampamentoId });
  const [filtro, setFiltro] = useState<FiltroStatus>("TODAS");
  const [busca, setBusca] = useState("");
  const [aberta, setAberta] = useState<Id<"inscricoesAcampamento"> | null>(null);
  ...
  if (acamp === null) {
    return (
      <Card>
        <CardContent className="p-6 text-muted-foreground">Acampamento não encontrado.</CardContent>
      </Card>
    );
  }
...
export default function AcampamentoDetalhePage() {
  ...
  return (
    ...
    >
      <HeaderLayout>
        <PageHeader title="Inscrições do acampamento" />
        <div className="mt-4">
