...
/**
 * Recebe a submissão do formulário público do retiro. Mesmo padrão do
...
export async function POST(req: NextRequest) {
  ...
  if (!body?.slug) {
    return NextResponse.json({ ok: false, error: "Retiro não informado" }, { status: 400 });
  }
  ...
  try {
    ...
    const result = await client.mutation(api.public.retiro.responder, {
      slug: body.slug!,
