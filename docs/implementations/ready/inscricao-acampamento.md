...
|--------|-----------------|
| `retiros` (NOVA) | Config do evento: slug, título, período (datas), ativa, janela de inscrição, **tabela de preços** (3 faixas etárias configuráveis `[{idadeMin, idadeMax, valor}]` — cortes e valores editáveis pelo admin —, adicional cama extra, adicional pet/dia, **valor da palestra por participante**), **estoque de quartos** (qtd de duplos e triplos disponíveis no hotel), `aportesFundo[]` `{valor, descricao, criadoPor, em}` (entradas avulsas no fundo solidário, sem vínculo com inscrição) |
| `inscricoesRetiro` (NOVA) | 1 doc = 1 grupo: responsável `{nome, whatsapp, membroId?}`, `participantes[]` `{nome, dataNascimento, membroId?, participaPalestras}`, hospedagem `{quartosDuplos, quartosTriplos, camasExtras, pets}`, extras `{colegaDeQuartoPreferido?, berco?, necessidadesEspeciais?, observacao?}`, pagamento `{formaPreferida, parcelasPreferidas?, cpfPagante}` (o que o inscrito pediu no form), **financeiro flexível**: `valorTabela` (snapshot do cálculo), `ajustes[]` `{tipo DESCONTO/CONTRIBUICAO_FUNDO, valor, motivo?, criadoPor, em}`, `recebimentos[]` `{valor, data, comprovanteUrl?, registradoPor, obs?}`, `planoPagamento[]` `{data, valor}` (previsão editável pela secretaria), status `ATIVA/LISTA_ESPERA/CANCELADA`, observacaoCancelamento?, lgpd, ipHash |
| `quartosRetiro` (NOVA) | Alocação: `{retiroId, tipo DUPLO/TRIPLO, identificacao?, ocupantes: [{inscricaoId, participanteIndex}]}` |
...
- Gestão (criar retiro, editar preços, respostas, baixa de pagamento, quartos): **`inscricoes:manage`** (já existe; secretaria/pastor/sec.exec já têm). Sem permissão nova.
...
- Preço por participante = faixa etária na **data de início do retiro** (idade calculada da dataNascimento). 3 faixas configuráveis (ex.: 0–4 isento, 5–10 reduzido, 11+ inteiro — cortes e valores livres).
...
- Admin define no retiro a quantidade de duplos e triplos do hotel.
...
2. **Recebimentos** = cada valor que chega (qualquer valor, qualquer data), com comprovante anexado
   (upload via `shared/files`, pasta `retiro-comprovantes/`; público exige allowlist no
...
3. **Fundo solidário do evento** = Σ contribuições (sobras de inscrições) + Σ **aportes
   ...
   com inscrição (doação de alguém que nem vai, verba da igreja): `retiros.aportesFundo[]`
...
|---------|------|-----------|
| `convex/schema.ts` | Modificar | 3 tabelas + índices (`by_slug`, `by_retiro`, `by_responsavel_whatsapp`) |
| `convex/retiro/{queries,mutations}.ts` | Criar | Admin CRUD (gated `inscricoes:manage`) |
| `convex/public/retiro.ts` | Criar | getBySlug, responder/editar (público + membro) |
| `convex/files/authz.ts` | Modificar | pasta `retiro-comprovantes/` p/ upload público limitado |
| `app/api/retiro/responder/route.ts` | Criar | ipHash (mesmo padrão de inscrições) |
| `app/(public)/(site)/retiro/[slug]/page.tsx` | Criar | Form público multi-participante (RHF+Zod, DatePickerBR, pré-preenchimento se logado) |
| `app/(ready)/admin/retiro/page.tsx` (+ sub-rotas) | Criar | Painel: config+preços, inscrições, pagamentos (baixa), quartos (DnD) |
| `features/retiro/*` | Criar | Componentes (form, tabela, quartos, parcelas) |
| `shared/constants/navigation.ts` | Modificar | Item "Retiro" (Secretaria), permission `inscricoes:manage` |
| `shared/components/layout/DevContext.tsx` | Modificar | Entradas novas |

## Ordem de Implementação

1. **Fase 1 — Backend base**: schema (commit isolado) → config do retiro + preços → mutation pública `responder` com cálculo do valor → testes do cálculo (faixas etárias, extras).
...
## Status da implementação (05/07/2026 — branch feature/retiro)

5 fases entregues na worktree: schema (3 tabelas), backend de config/preços/
cálculo, form público /retiro/[slug] (grupo + resumo ao vivo + pré-
