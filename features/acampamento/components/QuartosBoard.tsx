...
export function QuartosBoard({ acampamentoId }: { acampamentoId: Id<"acampamentos"> }) {
  // @ts-ignore Convex TS2589
  const dados = useQuery(api.acampamento.quartos.listarQuartos, { acampamentoId });
  const gerar = useMutation(api.acampamento.quartos.gerarQuartosDoPedido);
  const criar = useMutation(api.acampamento.quartos.criarQuarto);
  const remover = useMutation(api.acampamento.quartos.removerQuarto);
  const renomear = useMutation(api.acampamento.quartos.renomearQuarto);
  const mover = useMutation(api.acampamento.quartos.moverOcupante);
  ...
  async function onDragEnd(e: DragEndEvent) {
    ...
    const quartoId = alvo === "__sem_quarto__" ? null : (String(alvo) as Id<"quartosAcampamento">);
    await acao(() =>
      mover({
        acampamentoId,
        inscricaoId: inscricaoId as Id<"inscricoesAcampamento">,
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            onClick={async () => {
              const r = (await acao(() => gerar({ acampamentoId }))) as { criados: number } | undefined;
              if (r) {
                toast.success(
                  r.criados > 0
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuItem
                onClick={() => acao(() => criar({ acampamentoId, tipo: "DUPLO" }), "Quarto duplo criado")}
              >
                Duplo (2)
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => acao(() => criar({ acampamentoId, tipo: "TRIPLO" }), "Quarto triplo criado")}
              >
                Triplo (3)
              </DropdownMenuItem>
            </DropdownMenuContent>
