...
const isPublicRoute = createRouteMatcher(["/signin", "/ativar/(.*)", "/convite/(.*)", "/culto", "/inscricao/(.*)", "/livro/(.*)", "/convidado/(.*)", "/api/convidado-acesso", "/g/(.*)", "/subir-audio", "/quem-somos", "/trajetoria", "/agenda", "/visite", "/inscricoes", "/inscricoes/(.*)", "/privacidade", "/api/inscricoes/(.*)", "/opengraph-image(.*)"]);
