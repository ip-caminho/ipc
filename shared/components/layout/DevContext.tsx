...
const CONTEXT_MAP: Record<string, PageContext> = {
  "/dashboard": {
    ...
    arquivos: [
      ...
      "features/escalas/components/MinhaEscalaUnificada.tsx",
    ],
    queries: ["gravacoes.queries.getLatestAvisos", "membros.queries.birthdaysThisMonth", "escalas.queries.minhasEscalas"],
    mutations: ["membros.bootstrap.bootstrapAdmin"],
    componentes: ["BoletimCard", "TodaySection", "BirthdayList", "SectionLabel", "AniversariantesMesLista", "AvisosWidget (drawer)", "MinhaEscalaUnificada (drawer)", "EducacionalPaisWidget", "ChamadaWidget", "BootstrapForm"],
    notas: [
      "Hierarquia mobile-first: header compacto → BoletimCard (destaque) → seção 'Hoje' (grid 2 cols) → aniversariantes horizontais",
      "BoletimCard: bg #1a1a1a com badge ao vivo quando isDomingoWindow()=true; bg-secondary + 'Próximo culto' caso contrário",
      "TodaySection: cards Avisos + Minha escala abrem Drawer com widgets completos",
      "BirthdayList: scroll horizontal (scrollbar-none), avatares 44px com tempo relativo; 'Ver <mes> (N)' abre drawer com lista completa",
      "Todos os aniversariantes do mês ordenados por proximidade; passados com avatar bg-secondary neutro",
    ],
  },
  "/membros": {
  ...
  "/gravacoes": {
    ...
    notas: [
      "Tela unica: header breadcrumb + busca + chips de categoria + lista vertical",
      ...
      "Duracao derivada de fimSermao-inicioSermao quando disponivel (sermoes processados pela IA). Campo duracaoSegundos no schema fica como divida tecnica",
      "BibleBookFilter e lib/bible.ts deixaram de ser usados por esta tela",
    ],
  },
  "/gravacoes/nova": {
    nome: "Nova Gravacao",
