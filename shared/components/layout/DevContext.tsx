...
const CONTEXT_MAP: Record<string, PageContext> = {
  ...
  "/membros": {
    ...
    notas: [
      ...
      "Gestao de acesso/login movida para /admin/permissoes (aba Acesso, admin-only)",
    ],
  },
  "/membros/novo": {
    nome: "Novo Membro",
  ...
  "/membros/[id]": {
    ...
    componentes: ["MembroForm", "AtosPastoraisSection", "CargosHistoricoSection"],
    notas: [
      "Permissao: membros:read, membros:update",
      "Detalhe = dados pessoais/eclesiasticos + atos pastorais + cargos. Acesso/login e o papel (role) ficam em /admin/permissoes (admin-only)",
  ...
  "/admin/permissoes": {
    ...
    arquivos: [
      ...
      "convex/membros/acesso.ts",
      "types/auth.ts",
    ],
    queries: ["membros.acesso.getAcessosOverview", "membros.acesso.getAtividadeMembro"],
    mutations: [
      "membros.convites.generateInvite",
      "membros.acesso.gerarLink",
      "membros.acesso.resetarAcesso",
      "preferencias.rbac.updateMembroRole",
      "preferencias.rbac.setMembroPermission",
    ],
    componentes: ["PermissionMatrix", "AcessoPanel", "AtividadeMembroDrawer", "LinkConvidadoCard", "AdminGate"],
    notas: [
      "Somente admin (AdminGate + requireAdmin no backend)",
      "Tabs: Membros (role + permissions individuais), Acesso (ativar/resetar login, magic link, wa.me), Papeis (matriz role->permissao), Convites, Modo",
      "Aba Acesso = AcessoPanel movido de /membros: gera/reseta login (admin-only) + LinkConvidadoCard (link publico /convidado/<codigo>)",
      "gerarLink/resetarAcesso elevados de membros:update para admin (requireAdmin)",
    ],
  },
  "/admin/cadastro-vivo": {
    nome: "Cadastro Vivo",
